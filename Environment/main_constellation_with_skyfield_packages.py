...
class SkyfieldPhysicsEngine:
    ...
    def step_physics(self, state, action, iteration=1, delta_days=7.0):
        ...
        # action evaluation
        if action == "NO_OP":
            if crisis_before or final_cov < 92.0:
                reward -=300.0
            else:
                reward += 20.0
...
class SkyfieldADPSolver:
    ...
    def get_macro_state(self, state, kpis) -> tuple:
        ...
        return (oc, sp, h, cov, jam, pdop, ph, pend, plane_crisis)

    def get_post_decision_state(self, macro, action) -> tuple:
        oc,sp,h,cov,jam,pdop,ph,pend,plane_crisis = macro
        if   action == "ACTIVATE_SPARE"   and sp > 0:
            return (oc+1, sp-1, h, cov, jam, pdop, ph, pend, plane_crisis, "ACT")
        elif action == "LAUNCH_SATELLITE":
            return (oc,   sp,   h, cov, jam, pdop, ph, min(pend+1,2), plane_crisis, "LCH")
        elif action == "RETIRE_SATELLITE":
            return (max(oc-1,0), sp, h, cov, jam, pdop, ph, pend, plane_crisis, "RET")
        elif action == "REBALANCE_ORBITS":
            return (oc, sp, h, min(cov+1,2), jam, pdop, ph, pend, 0, "RBL")
        return (oc, sp, h, cov, jam, pdop, ph, pend, plane_crisis, "NOP")
...
if __name__ == "__main__":
    ...
    policy_path = os.path.join(OUTPUT_DIR, "ADP_policy.pkl")

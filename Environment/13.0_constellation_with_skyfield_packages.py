...
class SkyfieldPhysicsEngine:
    ...
    def get_valid_actions(self, state):
        ...
        crisis = self.is_plane_crisis(state)

        # Activate spare ONLY if dead satellites exist
        if any(s.health <= 0.0 for s in state.satellites) and state.spares_available > 0:
            valid.append("ACTIVATE_SPARE")

        # Launch only if spare pool is low
        if state.spares_available < (self.params.spare_capacity // 2):
            valid.append("LAUNCH_SATELLITE")

        # Retire only if degraded satellites exist
        if any(0.0 < s.health < 0.4 for s in state.satellites):
            valid.append("RETIRE_SATELLITE")

        # 🚨 CRITICAL FIX: Rebalance only in crisis
        if crisis or kpis['coverage_pct'] < 92:
    ...
    def step_physics(self, state, action, iteration=1, delta_days=7.0):
        ...
        if action == "NO_OP":
            ...
            if hasattr(self, 'prev_coverage'):
                if final_cov > self.prev_coverage + 3.0:
                    reward += 150.0
                elif final_cov < self.prev_coverage - 2.0:
                    reward -= 100.0
        ...
        urgency = 2.0 if len(attack_log) > 0 else 1.0
        reward *= urgency
...
class SkyfieldADPSolver:
    ...
    def get_macro_state(self, state, kpis) -> tuple:
        ...
        attack_severity = (
            0 if not state.was_under_attack else
            1 if kpis['coverage_pct'] >= 90 else
            2
        )
        ...
        return (oc, sp, h, cov, jam, pdop, ph, pend, plane_crisis, recent_attack, attack_severity)

    def get_post_decision_state(self, macro, action) -> tuple:
        oc,sp,h,cov,jam,pdop,ph,pend,plane_crisis,recent_attack,attack_severity = macro
        if   action == "ACTIVATE_SPARE"   and sp > 0:
            return (oc+1, sp-1, h, cov, jam, pdop, ph, pend, plane_crisis, recent_attack, attack_severity, "ACT")
        elif action == "LAUNCH_SATELLITE":
            return (oc,   sp,   h, cov, jam, pdop, ph, min(pend+1,2), plane_crisis, recent_attack, attack_severity, "LCH")
        elif action == "RETIRE_SATELLITE":
            return (max(oc-1,0), sp, h, cov, jam, pdop, ph, pend, plane_crisis, recent_attack, attack_severity, "RET")
        elif action == "REBALANCE_ORBITS":
            return (oc, sp, h, min(cov+1,2), jam, pdop, ph, pend, 0, recent_attack, attack_severity, "RBL")
        return (oc, sp, h, cov, jam, pdop, ph, pend, plane_crisis, recent_attack, attack_severity, "NOP")
